...
- **Resolucion aplicada**:
  ...
  5. **Servicio Unificado de Notificaciones por Correo (`sendEmailNotification`)**: La aplicacion cuenta con un servicio unificado en [`backend/index.js`](file:///c:/Users/nano9/OneDrive/Escritorio/Stuff/Professional/Business/FeryaelJustice/Developer/Projects/Web/HotelAuraDeMallorca/backend/index.js) configurado con Nodemailer para conectar con el servidor SMTP propio de Hostinger (`smtp.hostinger.com:465` con SSL), utilizando el buzon autenticado `contact@feryaeljustice.dev`. Dispone de soporte optativo para Brevo HTTP API si se define `BREVO_API_KEY`.
